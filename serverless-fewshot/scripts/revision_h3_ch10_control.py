# -*- coding: utf-8 -*-
#!/usr/bin/env python3
"""WP-H4 control: Azure-2019 cohort re-run with feature channel 10 zeroed.

The Huawei release has no app/owner grouping, so channel 10 (app_corate) is
identically zero there.  This script quantifies what that deviation costs by
imposing the same handicap on the Azure-2019 cohort run, everything else held
fixed (same checkpoint, prototypes, cohort, seeds, arms, decision layer).  If
the 2019 verdict survives the handicap, the Huawei numbers cannot be an
artifact of the missing channel.

Base protocol (verbatim from revision_a4_crosstrace.py):
...
Output: results/runs/revision_h3_ch10_control.json
"""
...
def main():
        ...
        for w in range(W):
            ...
            for (fi, t0) in pts:
                ...
                if t < L:
                    ...
                    win = np.array(features[fi, t - L:t], dtype=np.float32)
                win[:, 10] = 0.0          # Huawei handicap: no app co-rate
    ...
    out = {"config": "cross-trace 2019 cohort, feature channel 10 "
                     "(app_corate) zeroed -- Huawei-deviation control",
           "control": "app_corate_zeroed",
    ...
    path = RUNS_DIR / "revision_h3_ch10_control.json"
