...
launch_one() {
  ...
  if pgrep -f "revision_r21_des_runner_resume.py --jobs ${jobs} --out ${out}" >/dev/null; then
    echo "${name}: already running"
    return
  fi
  nohup env PYTHONPATH="$PYTHONPATH_DES" python3.13 scripts/revision_r21_des_runner_resume.py \
    --jobs "$jobs" --out "$out" > "$log" 2>&1 &
