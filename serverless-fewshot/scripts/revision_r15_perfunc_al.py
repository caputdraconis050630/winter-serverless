# -*- coding: utf-8 -*-
#!/usr/bin/env python3
"""R15: per-function adaptation-lag distribution on the 2019 natural cohort.

The archived campaign (revision_a4_crosstrace.json) reports AL only at cohort
level, because per-arm per-function per-minute rolling traces were never
archived - the paper lists this as a limitation.

This script is revision_a4_crosstrace.py with one change: the per-function
rolling (cold, total) arrays are accumulated instead of only their cohort sum,
so AL(eps) can be computed per function with the canonical definition from
src/sim/simulator.py. Everything else - cohort rule, seeds, checkpoint,
constants, decision layer - is byte-identical to the archived run, so the
cohort-level numbers double as a reproduction gate.

Output: results/runs/revision_r15_perfunc_al.json
"""
...
def main():
    ...
    for rho in RHOS:
        ...
        for m, (pw_m, ka_m) in dec.items():
            ...
            # R15: per-function rolling accumulators (the only added state)
            fr_c = np.zeros((F_n, W)); fr_n = np.zeros((F_n, W))
            for f in range(F_n):
                for seed in DES_SEEDS:
                    ...
                    fr_c[f] += r["roll_cold"]; fr_n[f] += r["roll_total"]
            ...
            # R15: per-function AL(eps) at three tolerances
            per_fn_al = {}
            for eps in (0.05, 0.1, 0.2):
                vals = []
                for f in range(F_n):
                    rc = rolling_csr(fr_c[f], fr_n[f], window=15)
                    al = compute_adaptation_lag(list(enumerate(rc.tolist())),
                                                event_tick=0, epsilon=eps,
                                                window=15, stability_window=30)
                    vals.append(al)
                per_fn_al[f"eps{eps}"] = vals
            rho_out[m] = {"overall_csr": float(roll_c.sum() / max(roll_n.sum(), 1e-9)),
                          "per_function_al": per_fn_al,
    ...
    path = RUNS_DIR / "revision_r15_perfunc_al.json"
