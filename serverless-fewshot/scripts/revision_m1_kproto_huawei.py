# -*- coding: utf-8 -*-
#!/usr/bin/env python3
"""M1/E1: decision-level prototype-granularity sweep on the Huawei cohort.

Cross-provider counterpart to revision_m1_kproto.py (Azure 2021 injection).

Why
---
On the Huawei natural new-deployment cohort the archived k=16 prototype
assignment collapses *completely*: all 76 functions map to a single cluster
(revision_h1_huawei_cohort.json 'assign_hist_k0' = [0,0,0,0,0,76,0,...],
entropy 0 bits), and the prototype increment over unbiased few-shot
adaptation is ~0 there (mean_diff -0.035 / -0.031 / +0.325 cold starts per
function at rho = 1 / 10 / 100, Wilcoxon p = 0.42 / 0.63 / 0.10).

The Azure sweep showed the granularity effect is conditional and that
collapse is a *symptom of k being too large*: at k=4 the assignment spreads
(effective 3.73 of 4 clusters) and beats a single pooled prior, while k=16
collapses (effective 1.77 of 16) and is statistically indistinguishable from
k=1.  If that reading is right, the Huawei collapse should also lift at
smaller k -- and the prototype increment should reappear.  If it does not,
the honest conclusion is that the collapse is Azure-specific and the Huawei
cohort simply has one behaviour type.

Either outcome is reportable; this run decides which.

Protocol
--------
Identical to revision_h1_huawei_cohort.py in every registered respect
...
Writes results/runs/revision_m1_kproto_huawei.json.  Touches no archive.
"""
...
from scripts.revision_h1_huawei_cohort import (                # noqa: E402
...
KS = [1, 4, 16, 64]
ARMS_PRED = ["noproto"] + [f"k{k}" for k in KS] + [f"k{k}_zero" for k in KS]
BOOT_SEED = 0
...
def main():
    ...
    fp_path = PROCESSED_DIR / "first_present.npy"
    first_present = np.load(fp_path) if fp_path.exists() else None
    pts, funnel = find_natural_cohort(counts, first_present)
    F_n = len(pts)
    print(f"natural cohort: {F_n} functions; funnel={funnel}")

    trainer, biased_head = load_trainer(nF, seed=0)   # 2021 checkpoint

    # ---- one prototype set per k (monkeypatch the module global) ----
    protos = {}
    k_orig = eab.N_CLUSTERS
    try:
        for k in KS:
            eab.N_CLUSTERS = k
            cen, pw = eab.build_prototypes(trainer, biased_head,
                                           feat21, cnt21, spl21["s2_train"])
            protos[k] = (cen.to(DEVICE), pw.to(DEVICE))
            print(f"  built k={k}: centroids {tuple(cen.shape)} "
                  f"weights {tuple(pw.shape)}")
    finally:
        eab.N_CLUSTERS = k_orig
    ...
    for w in range(W):
        ...
        for k in KS:
            ...
            if w == 0:
                assign0[k] = ids.cpu().numpy().copy()
            Wp = pw_g[ids]
            W_p = Wp.clone() if w == 0 else torch.linalg.solve(A, PhiTY + lam * Wp)
            preds[f"k{k}"][:, w] = (phi_now.unsqueeze(1) @ W_p).squeeze(1).cpu().numpy()
            preds[f"k{k}_zero"][:, w] = (phi_now.unsqueeze(1) @ Wp).squeeze(1).cpu().numpy()
        if (w + 1) % 120 == 0:
    ...
    assign_out = {}
    for k in KS:
        hist = np.bincount(assign0[k], minlength=k)
        p = hist / hist.sum()
        ...
        print(f"  k={k:2d}: top-share {hist.max()/hist.sum():.2f} "
    ...
    for k in KS:                       # gate v3 per granularity
        rates[f"gated_v3_k{k}"] = np.where(
            zero_h, rates[f"k{k}_zero"],
            np.where(mid, rates["B4a_ewma"], rates[f"k{k}"]))
    ...
    a_path = RUNS_DIR / "revision_h1_huawei_cohort.json"
    if a_path.exists():
    ...
    path = RUNS_DIR / "revision_m1_kproto_huawei.json"
