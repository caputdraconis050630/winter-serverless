# -*- coding: utf-8 -*-
#!/usr/bin/env python3
"""WP-H2 (regime-map cell 1): CROSS-PROVIDER natural onboarding on Huawei.

Identical protocol to revision_a4_crosstrace.py (the Azure-2019 cohort run),
with the evaluation trace swapped to the Huawei Public 2023 release:

  body + prototypes meta-learned on the Azure *2021* trace
  (results_azure2021/runs/best_anil_ridge_s2_s0.pt, prototypes from 2021
  s2_train) -> evaluated on Huawei natural new deployments.

Temporal/leakage integrity is unconditional here: the predictor never saw
this provider, let alone these functions. Cohort definition, arms, seeds,
TAU semantics, and the gate thresholds are the registered constants -- no
retuning for the new trace.

Stronger cohort guarantee than on Azure: the Huawei release marks a function
as absent (NaN) until it is deployed, so qualifying functions are verified
absent from the trace before t0, not merely silent. The funnel and the
absent-before-t0 count are recorded in the output.

Output: results/runs/revision_h1_huawei_cohort.json
"""
...
def find_natural_cohort(counts, first_present=None):
    N, T = counts.shape
    picked = []
    funnel = {"universe": N, "any_invocation": 0, "t0_after_idle_prefix": 0,
              "t0_before_trace_end": 0, "day1_invocations": 0}
    for f in range(N):
        row = np.asarray(counts[f])
        nz = np.flatnonzero(row)
        if len(nz) == 0:
            continue
        funnel["any_invocation"] += 1
        t0 = int(nz[0])
        if t0 < MIN_IDLE_PREFIX:
            continue
        funnel["t0_after_idle_prefix"] += 1
        if t0 >= T - 1440:
            continue
        funnel["t0_before_trace_end"] += 1
        if row[t0:t0 + 1440].sum() < MIN_DAY1_INV:
            continue
        funnel["day1_invocations"] += 1
        picked.append((f, t0))
    funnel["qualifying"] = len(picked)
    if first_present is not None:
        funnel["absent_from_trace_before_t0"] = int(sum(
            1 for (f, t0) in picked if first_present[f] >= t0))
...
def main():
    ...
    out = {"config": "cross-provider (2021-trained body+prototypes -> "
                     "Huawei Public 2023 cohort)",
           ...
           "duration_model": "huawei_private_2023 (imputed; see meta.json)",
           "app_corate_channel": "identically zero (no app grouping)",
    ...
    path = RUNS_DIR / "revision_h1_huawei_cohort.json"
