# -*- coding: utf-8 -*-
#!/usr/bin/env python3
"""Revision A4(2): NATURAL onboarding cohort on Azure-2019.

Cohort: functions in the 2019 active pool whose first invocation occurs
mid-trace — zero invocations for the first 3 days (t < 4320), first arrival
t0 in [4320, T-1440), and >= 30 invocations in the following 24h. These are
genuine new deployments, not synthetically truncated histories. Capped at
100 functions (rng seed 42, a priori).

Evaluation mirrors the synthetic-injection onboarding (phase63 / revision_a1
semantics, TAU rate extraction, decisions_from_rates, simulate_function,
DES seeds {0,1,2}), arms {A5_proto, A5_noproto, B4a_ewma, B1, Oracle},
rhos {1, 10, 100}. 2019-trained checkpoint best_anil_ridge_s2_s0.pt;
prototypes built from s2_train (PROTO_TRAIN_CAP=2000, k=16) via the
memmap-safe pipeline of eval_adapt_biased.

Run with: SF_DATA_DIR=processed_2019 (25GB features stay memmapped).
Output: results/runs/revision_a4_natural_onboarding.json
"""
...
RHOS = [1.0, 10.0, 100.0]
COHORT_CAP = 100
MIN_IDLE_PREFIX = 3 * 1440       # zero invocations before day 3
MIN_DAY1_INV = 30
QL = np.linspace(0.05, 0.95, N_QUANTILES)
TAU = newsvendor_quantile(10.0)   # same fixed rate-fractile as phase63
...
def main():
    ...
    print(f"natural cohort: {F_n} functions "
          f"(first-invocation t0 median {np.median([t for _, t in pts]):.0f} min)")
    ...
    # ---- embeddings phi[F, W, d] from memmap windows ----
    trainer.body.eval()
    ...
    eye = torch.eye(64)
    n_out = N_QUANTILES
    preds = {a: np.zeros((F_n, W, n_out), dtype=np.float32)
             for a in ["A5_proto", "A5_noproto"]}
    import torch.nn.functional as Fun
    assign0 = None
    for w in range(W):
        phi_now = phi_all[:, w]
        sims = Fun.cosine_similarity(phi_now.unsqueeze(1),
                                     centroids.unsqueeze(0), dim=2)
        ids = sims.argmax(dim=1)
        if w == 0:
            assign0 = ids.numpy().copy()
        Wp = pw_all[ids]
        if w == 0:
            W_p, W_n = Wp.clone(), torch.zeros(F_n, 64, n_out)
        else:
            phi_s = phi_all[:, :w]
        ...
        preds["A5_proto"][:, w] = (phi_now.unsqueeze(1) @ W_p).squeeze(1).numpy()
        preds["A5_noproto"][:, w] = (phi_now.unsqueeze(1) @ W_n).squeeze(1).numpy()
    ...
    out = {"n_functions": F_n, "cohort_cap": COHORT_CAP,
    ...
    for rho in RHOS:
        ...
        ours = np.array(rho_out["A5_proto"]["func_cold60"])
        paired = {}
        for b in ["A5_noproto", "B4a_ewma", "B1_fixed_keepalive"]:
            base = np.array(rho_out[b]["func_cold60"])
            diff = base - ours
            nz = diff[diff != 0]
        ...
        print(f"rho={rho}: " + " ".join(
            ...
            f" | paired vs noproto p={paired['A5_noproto']['wilcoxon_p']:.4f} "
            f"vs B1 p={paired['B1_fixed_keepalive']['wilcoxon_p']:.4f}")

    path = RUNS_DIR / "revision_a4_natural_onboarding.json"
