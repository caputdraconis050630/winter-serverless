"""Fixed-alpha replay with exact whole-schedule reuse on heavy source cases."""
import argparse
...
from simulator_events import advance,new_state,execution_curve,convert_heap_checkpoint
from run import windows_for,aggregate
OUT=ROOT/"results/fixed_ewma_v1/replay"
SOURCE=ROOT/"results/lstm_comparison_v1"
...
def run(name,workers=6):
    ...
    source=SOURCE/"cases"/meta["name"]
    source_actions=[]
    if (source/"seed_functions").exists():
        sm=read_json(source/"case.json");sc=read_json(source/"execution.json")
        assert sm["data_sha256"]==meta["data_sha256"] and sm["seeds"]==meta["seeds"]
        assert sm["event_key_prefix"]==meta["event_key_prefix"]
        for version in ('heap','readylist'):
            expected=sc.get(f'migrated_{version}_contract_sha256')
            if expected:assert digest(source/f'execution_{version}.json')==expected
        for ai,key in enumerate(sc["actions"]):
            if sm["actions"][key]["method"] not in ("WINTER","WINTER_G","EWMA_0.3","No_handoff"):continue
            p=source/sm["actions"][key]["file"]
            assert digest(p)==sm["actions"][key]["sha256"]
            with np.load(p) as z:source_actions.append((ai,z["q"],z["ttl"]))
    source_hash=digest(source/"execution.json") if source_actions else None
    contract=dict(case_sha256=digest(directory/"case.json"),actions=names,seeds=meta["seeds"],windows=windows,
        metrics=list(METRICS),memory_gb=.25,cap=200,simulator_sha256=digest(HERE/"simulator_events.py"),
        residency_helpers_sha256=digest(HERE/"simulator_fast.py"),
        reference_simulator_sha256=digest(HERE/"simulator.py"),
        ...
        scheduling="function/seed jobs; state checkpoint every 300 seconds; common warm-completion order and equivalent residency ledger",
        reused_source_execution_sha256=source_hash,
        migrated_heap_contract_sha256=digest(directory/"execution_heap.json") if (directory/"execution_heap.json").exists() else None,
        migrated_readylist_contract_sha256=digest(directory/"execution_readylist.json") if (directory/"execution_readylist.json").exists() else None)
    ...
    def job(f,s):
        ...
        if path.exists():
            with np.load(path) as z:assert str(z["contract_sha256"]) in (contract_hash,contract["migrated_heap_contract_sha256"],contract["migrated_readylist_contract_sha256"])
            return f,s
        unique=[];mapping=[];seen={}
        ...
        reused={}
        source_seed=source/"seed_functions"/f"{f:05d}_{s:02d}.npz"
        if source_actions:
            assert source_seed.exists()
            # Match full q/TTL sequences before consulting any saved outcome.
            available={}
            for ai,qa,ta in source_actions:
                q,t=np.ascontiguousarray(qa[f]),np.ascontiguousarray(ta[f])
                available.setdefault(hashlib.sha256(q.tobytes()+t.tobytes()).digest(),ai)
            with np.load(source_seed) as z:
                assert str(z["contract_sha256"]) in (source_hash,
                    sc.get('migrated_heap_contract_sha256'),sc.get('migrated_readylist_contract_sha256'))
                for key,j in seen.items():
                    if key in available:
                        oi=z["mapping"][available[key]]
                        reused[j]=(z["cold"][oi].copy(),z["idle"][oi].copy(),z["stats"][oi].copy())
        states=[new_state(horizon) for _ in unique]
        resume=partial/f"{f:05d}_{s:02d}.npz"
        offset=0;execution=0.
        if resume.exists():
            with np.load(resume) as z:
                old_hash=str(z["contract_sha256"])
                assert old_hash in (contract_hash,contract["migrated_heap_contract_sha256"],contract["migrated_readylist_contract_sha256"])
                ...
                if old_hash==contract["migrated_heap_contract_sha256"]:states=[convert_heap_checkpoint(st) for st in states]
        start=last_report=last_save=time.monotonic()
        counts=data["counts"][f]
        chunks=event_chunks(counts,data["duration_mean"][f],data["duration_std"][f],seed,
                                       meta["event_key_prefix"]+str(data["keys"][f])) if len(reused)<len(unique) else ()
        for lo,hi,tape in chunks:
            if hi<=offset:continue
            assert lo>=offset
            execution+=.25*tape[2].sum()
            baseline=execution_curve(tape[1],tape[2],horizon)
            completions=tape[1]+tape[2];order=np.argsort(completions,kind="stable")
            for j,(q,ttl) in enumerate(unique):
                if j not in reused:advance(*tape,q[lo:hi],ttl[lo:hi],*states[j],lo,hi==horizon,.25,200,baseline,order,completions)
            now=time.monotonic()
        ...
        for j,state in enumerate(states):
            if j in reused:
                c,i,st=reused[j];cold.append(c);idle.append(i);stats[j]=st
                continue
            out=state[0]
        ...
        save_npz(path,mapping=np.array(mapping),cold=np.stack(cold),idle=np.stack(idle),stats=stats,
                 ...
                 reused_actions=np.array(sorted(reused),dtype=np.int64))
