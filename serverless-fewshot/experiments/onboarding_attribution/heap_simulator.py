"""Indexed-heap acceleration of the independently verified interval DES."""
import numpy as np
from numba import njit

from simulator import _close
...
def simulate_fast(ptr,arrival,duration,cold_init,warm_init,targets,ttl,memory=.25,cap=200,legacy_ttl=False):
    if legacy_ttl:
        # Only the bridge uses the deliberately archived behavior.
        from_dummy=0
        raise ValueError("Use the reference kernel for legacy TTL")
    out=np.zeros((5,8))
    heap=np.zeros((3,cap),dtype=np.int64)
    pos=np.full((3,cap),-1,dtype=np.int64)
    size=np.zeros(3,dtype=np.int64)
    key=np.zeros((3,cap))
    birth=np.zeros(cap); ready=np.zeros(cap); free=np.zeros(cap)
