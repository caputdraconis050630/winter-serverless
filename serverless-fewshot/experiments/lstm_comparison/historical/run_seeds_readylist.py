...
from simulator_fast import advance,new_state,execution_curve,convert_heap_checkpoint
...
def run(name,workers=6):
    ...
    contract=dict(case_sha256=digest(directory/"case.json"),actions=names,seeds=meta["seeds"],windows=windows,
        metrics=list(METRICS),memory_gb=.25,cap=200,simulator_sha256=digest(HERE/"simulator_fast.py"),
        ...
        scheduling="function/seed jobs; state checkpoint every 300 seconds; equivalent ready-list and residency ledger",
    ...
    def job(f,s):
        ...
        if resume.exists():
            with np.load(resume) as z:
                ...
                assert old_hash in (contract_hash,contract["migrated_heap_contract_sha256"])
                ...
                if old_hash!=contract_hash:states=[convert_heap_checkpoint(st) for st in states]
        ...
        for lo,hi,tape in event_chunks(counts,data["duration_mean"][f],data["duration_std"][f],seed,
            ...
            for j,(q,ttl) in enumerate(unique):advance(*tape,q[lo:hi],ttl[lo:hi],*states[j],lo,hi==horizon,.25,200,baseline)
            now=time.monotonic()
